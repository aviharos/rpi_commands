...
def post_to_IoT_agent(session: requests.Session, req: dict):
    logger.debug(f"post_to_IoT_agent: req: {req}")
    logger.debug(f"post_to_IoT_agent: url: http://{MOMAMS_HOST}:{IOTAGENT_HTTP_PORT}")
    res = session.post(url=f"http://{MOMAMS_HOST}:{IOTAGENT_HTTP_PORT}", headers={"Content-Type": "application/json"}, json=req, timeout=TIMEOUT)
